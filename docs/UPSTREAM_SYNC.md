...
| Teams `graph` primitives `call_teams_graph_api` host gate (vercel/chat#8c71411, chat@4.31) | `call_teams_graph_api` (`teams/graph/__init__.py`) gates an absolute `path_or_url` through `is_trusted_graph_url` **before** resolving/attaching the `Bearer` token, raising `ValueError` for any host other than `graph.microsoft.com` over `https`. The same gate guards every `@odata.nextLink` followed by `follow_teams_graph_next_link` / `paginate_teams_graph`, so an attacker-controlled pagination cursor cannot redirect the authenticated GET. `_TRUSTED_GRAPH_HOSTS` pins to the bare `graph.microsoft.com` host exactly (a Graph API call never targets a subdomain). | Upstream `graph/client.ts` `callTeamsGraphApi` builds the request URL (relative path resolved against `graphUrl`, or an absolute `pathOrUrl`/`@odata.nextLink` used verbatim) and GETs it with `Authorization: Bearer ${token}` — no scheme/host validation | Token-leak / SSRF guard, Python-first hardening with no upstream equivalent. The Graph access token (acquired at the `https://graph.microsoft.com/.default` scope) is highly privileged; these primitives are SDK-free and accept caller-supplied input, and crucially `@odata.nextLink` is server-returned data that upstream follows **verbatim** — a Graph response (or a parsed-but-unverified one) carrying a hostile `@odata.nextLink` would otherwise exfiltrate the token to an attacker host on the next page fetch. `is_trusted_graph_url` requires scheme `https` and host exactly `graph.microsoft.com`; the gate runs before any token request, so an untrusted host triggers **zero** outbound calls. Enforces `CLAUDE.md`'s "Validate external URLs before requests (SSRF)" rule, mirroring the Slack `fetch_slack_file` and Teams `api` `call_teams_connector_api` rows above. Regression coverage: `tests/test_teams_graph_primitive.py::TestTeamsGraphSsrfDivergence`. |
